...
describe("maskBrand", () => {
  describe("placeholder at attempt 1", () => {
    it('shows "???" placeholder at attempt 1', () => {
      expect(maskBrand("Chanel", 1)).toBe(GENERIC_PLACEHOLDER.repeat(3));
      expect(maskBrand("Dior", 1)).toBe(GENERIC_PLACEHOLDER.repeat(3));
    ...
    it('shows "???" placeholder at attempt 0 or negative', () => {
      expect(maskBrand("Chanel", 0)).toBe(GENERIC_PLACEHOLDER.repeat(3));
      expect(maskBrand("Dior", -1)).toBe(GENERIC_PLACEHOLDER.repeat(3));
    });
  });
  ...
  describe("progressive reveal (attempts 2-6)", () => {
    it("shows full underscores at attempt 2 (0% reveal)", () => {
      ...
      expect(maskBrand("Dior", 2)).toBe(MASK_CHAR.repeat(4));
      expect(maskBrand("Chanel", 2)).toBe(MASK_CHAR.repeat(6));
      expect(maskBrand("Tom Ford", 2)).toBe(
        `${MASK_CHAR.repeat(3)} ${MASK_CHAR.repeat(4)}`,
      );
    });
  ...
  describe("multi-word brands", () => {
    it("preserves spaces in multi-word brands at attempt 2", () => {
      expect(maskBrand("Yves Saint Laurent", 2)).toBe(
        `${MASK_CHAR.repeat(4)} ${MASK_CHAR.repeat(5)} ${MASK_CHAR.repeat(7)}`,
      );
      expect(maskBrand("Jean Paul Gaultier", 2)).toBe(
        `${MASK_CHAR.repeat(4)} ${MASK_CHAR.repeat(4)} ${MASK_CHAR.repeat(8)}`,
      );
      expect(maskBrand("Tom Ford", 2)).toBe(
        `${MASK_CHAR.repeat(3)} ${MASK_CHAR.repeat(4)}`,
      );
    ...
    it("preserves spaces while revealing letters progressively", () => {
      ...
      expect(result3).toMatch(
        new RegExp(`^[a-z${MASK_CHAR}]+ [a-z${MASK_CHAR}]+$`, "i"),
  ...
  describe("special characters and hyphens", () => {
    it("handles brands with hyphens", () => {
      ...
      // revealLetters masks all non-space characters as MASK_CHAR, including hyphens
      expect(result2).toBe(MASK_CHAR.repeat(9)); // "Jean-Paul" = 9 chars, all masked
    ...
    it("handles brands with apostrophes", () => {
      ...
      expect(result2).toMatch(new RegExp(`^${MASK_CHAR}+$`)); // All non-space chars masked
  ...
  describe("edge cases", () => {
    it("handles empty string", () => {
      expect(maskBrand("", 1)).toBe(GENERIC_PLACEHOLDER.repeat(3));
      expect(maskBrand("", 2)).toBe("");
      expect(maskBrand("", 6)).toBe("");
    });

    it("handles single character brand", () => {
      expect(maskBrand("X", 1)).toBe(GENERIC_PLACEHOLDER.repeat(3));
      expect(maskBrand("X", 2)).toBe(MASK_CHAR);
      expect(maskBrand("X", 6)).toBe("X");
    });

    it("handles very long brand name", () => {
      const longBrand = "Abcdefghijklmnopqrstuvwxyz";
      expect(maskBrand(longBrand, 1)).toBe(GENERIC_PLACEHOLDER.repeat(3));
      expect(maskBrand(longBrand, 2)).toBe(MASK_CHAR.repeat(26));
  ...
  describe("reveal percentages match specification", () => {
    it("uses correct reveal percentages per attempt", () => {
      // Attempt 1: ???
      expect(maskBrand("Test", 1)).toBe(GENERIC_PLACEHOLDER.repeat(3));

      // Attempt 2: 0%
      expect(maskBrand("Test", 2)).toBe(MASK_CHAR.repeat(4));

      // Attempt 3: 15% (at least 1 char visible for 4-char word)
      const result3 = maskBrand("Test", 3);
      expect(result3).not.toBe(MASK_CHAR.repeat(4)); // Not fully masked
      ...
      expect(result4).not.toBe(MASK_CHAR.repeat(4));
  ...
  describe("realistic perfume brand examples", () => {
    it("masks luxury brand names progressively", () => {
      ...
      for (const brand of brands) {
        expect(maskBrand(brand, 1)).toBe(GENERIC_PLACEHOLDER.repeat(3));
        expect(maskBrand(brand, 2)).toMatch(new RegExp(`^[${MASK_CHAR} ]+$`)); // Only MASK_CHAR and spaces
        expect(maskBrand(brand, 2)).toMatch(new RegExp(`^[${MASK_CHAR} ]+$`)); // Only MASK_CHAR and spaces
...
describe("maskYear", () => {
  ...
  describe("digit-by-digit reveal", () => {
    it(`shows "${MASK_CHAR.repeat(4)}" at attempt 1`, () => {
      expect(maskYear(1979, 1)).toBe(MASK_CHAR.repeat(4));
      expect(maskYear(2015, 1)).toBe(MASK_CHAR.repeat(4));
      expect(maskYear(1921, 1)).toBe(MASK_CHAR.repeat(4));
    });

    it("reveals first digit at attempt 2", () => {
      expect(maskYear(1979, 2)).toBe(`1${MASK_CHAR.repeat(3)}`);
      expect(maskYear(2015, 2)).toBe(`2${MASK_CHAR.repeat(3)}`);
      expect(maskYear(1921, 2)).toBe(`1${MASK_CHAR.repeat(3)}`);
    });

    it("reveals two digits at attempt 3", () => {
      expect(maskYear(1979, 3)).toBe(`19${MASK_CHAR.repeat(2)}`);
      expect(maskYear(2015, 3)).toBe(`20${MASK_CHAR.repeat(2)}`);
      expect(maskYear(1921, 3)).toBe(`19${MASK_CHAR.repeat(2)}`);
    });

    it("reveals three digits at attempt 4", () => {
      expect(maskYear(1979, 4)).toBe(`197${MASK_CHAR}`);
      expect(maskYear(2015, 4)).toBe(`201${MASK_CHAR}`);
      expect(maskYear(1921, 4)).toBe(`192${MASK_CHAR}`);
    });
  ...
  describe("edge cases", () => {
    it("handles 3-digit years (edge case, unlikely but possible)", () => {
      ...
      expect(maskYear(999, 1)).toBe(MASK_CHAR.repeat(4));
      expect(maskYear(999, 2)).toBe(`9${MASK_CHAR.repeat(3)}`);
      expect(maskYear(999, 3)).toBe(`99${MASK_CHAR.repeat(2)}`);
      expect(maskYear(999, 4)).toBe(`999${MASK_CHAR}`);
      expect(maskYear(999, 5)).toBe("999");
    });

    it("handles 5-digit years (edge case, unlikely)", () => {
      expect(maskYear(12_345, 1)).toBe(MASK_CHAR.repeat(4));
      expect(maskYear(12_345, 2)).toBe(`1${MASK_CHAR.repeat(3)}`);
      expect(maskYear(12_345, 3)).toBe(`12${MASK_CHAR.repeat(2)}`);
      expect(maskYear(12_345, 4)).toBe(`123${MASK_CHAR}`);
      expect(maskYear(12_345, 5)).toBe("12345");
    });

    it("handles attempt 0 or negative (shows full mask)", () => {
      expect(maskYear(1979, 0)).toBe(MASK_CHAR.repeat(4));
      expect(maskYear(2015, -1)).toBe(MASK_CHAR.repeat(4));
    });
  });
  ...
  describe("realistic perfume year examples", () => {
    it("masks classic perfume years (1900s)", () => {
      ...
      for (const { year } of classicYears) {
        expect(maskYear(year, 1)).toBe(MASK_CHAR.repeat(4));
        expect(maskYear(year, 2)).toBe(`1${MASK_CHAR.repeat(3)}`);
        expect(maskYear(year, 3)).toBe(`19${MASK_CHAR.repeat(2)}`);
    ...
    it("masks modern perfume years (2000s)", () => {
      ...
      for (const { year } of modernYears) {
        expect(maskYear(year, 1)).toBe(MASK_CHAR.repeat(4));
        expect(maskYear(year, 2)).toBe(`2${MASK_CHAR.repeat(3)}`);
        expect(maskYear(year, 3)).toBe(`20${MASK_CHAR.repeat(2)}`);
        expect(maskYear(year, 5)).toBe(year.toString());
      }
    });
  });
