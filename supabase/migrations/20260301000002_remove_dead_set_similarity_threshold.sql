-- Migration: Remove dead SET pg_trgm.similarity_threshold
-- Date: 2026-03-01
--
-- The EXECUTE format('SET pg_trgm.similarity_threshold = %s', fuzzy_threshold)
-- line was dead code — we never use the % operator (which reads this GUC).
-- Instead, we compare explicitly: extensions.similarity(...) > fuzzy_threshold.
-- Removing it eliminates one unnecessary executor roundtrip per search call.
